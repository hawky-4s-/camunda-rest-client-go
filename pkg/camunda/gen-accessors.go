...
var (
	...
	blacklistRegexStruct = []string{
		".*Dto",
		".*Builder",
	}
)

func isBlackListedStruct(ts *ast.TypeSpec) bool {
	for _, regex := range blacklistRegexStruct {
		matched, err := regexp.MatchString(regex, ts.Name.Name)
		if err != nil {
			log.Fatal(matched, err)
		}
		if matched {
			log.Println("Ignoring type:", ts.Name.Name)
			return true
		}
	}
	return false
